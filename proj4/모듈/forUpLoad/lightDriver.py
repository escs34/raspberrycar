...
def lineTracking(direction):
	...
	if direction == 'l':	
		...
		if bit==24 or bit==16 or bit==0:
			car.engine(True, False, lSpeed, rSpeed)#False, lSpeed, rSpeed)
			count=0
			while not lightTrack.bit8():# (0<lightTrack.navigator()<6):
				count+=1
				print(count)
				if count%1400==0:
					car.engine(True, False, lSpeed, rSpeed)
			...
			car.engine(False, True, lSpeed, rSpeed)#False, True, lSpeed, rSpeed)
			count=0
			while not (lightTrack.bit4()):
				count+=1
				print(count)
